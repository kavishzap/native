...
const ConcertManagement = () => {
    ...
    const openModal = (concert: any = null) => {
        if (concert) {
            setFormData({
                ...
                concert_region: concert.concert_region,
    ...
    const handleImageUpload = async (files: FileList | null) => {
        ...
        reader.onload = () => {
            ...
            setFormData(prev => ({ ...prev, concert_image: base64 }));
        };
    ...
    const handleSubmit = async () => {
        ...
        if (
            ...
            !concert_location_name || !concert_region || !concert_start_time ||
            !concert_end_time || !concert_type || !concert_description ||
            !concert_status || !concert_image
        ) {
            Swal.fire('Validation Error', 'Please fill in all fields before submitting.', 'warning');
            return;
        }
        ...
        let error;
        if (editingId) {
            ({ error } = await supabase.from('concerts').update(payload).eq('id', editingId));
        } else {
            ({ error } = await supabase.from('concerts').insert([payload]));
        }

        if (!error) {
            Swal.fire('Success', 'Concert saved successfully.', 'success');
            fetchConcerts();
            setModalOpen(false);
            setEditingId(null); // ✅ reset after submit
        } else {
    ...
    const deleteConcert = async (id: number | null) => {
        if (id === null) return; // prevent invalid calls
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            value={formData.concert_region}
                                            onChange={(e) => setFormData({ ...formData, concert_region: e.target.value })}
                                        >
                                    ...
                                    <div className="col-span-full">
                                        <label className="block text-sm text-gray-700 dark:text-gray-200 mb-2">Upload Images (Max 3)</label>
